...
function Table.Lock(Tab)
	...
	function Metatable:__index(Index)
		...
		return Value == nil and require(script.Parent.Debug).Error("!%q does not exist in read-only table", ModuleName, Index) or Value
	end

	function Metatable:__newindex(Index, Value)
		require(script.Parent.Debug).Error("!Cannot write %s to index [%q] of read-only table", ModuleName, Value, Index)
	end
