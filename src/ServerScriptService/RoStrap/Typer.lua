	...
	if not PotentialTypes[0] then -- It was already transformed if written to 0, no haxing pls
		...
		for Name in next, PotentialTypes do
			...
			elseif NameType ~= "number" then
				require(script.Parent.Debug).Error("Key-Value pairs should be in the form [string Name] = function, got %s", Name)
			end
		end
		...
		while i < AmountPotentialTypes do
			...
			if type(PotentialType) ~= "string" then
				require(script.Parent.Debug).Error("PotentialTypes in the array section must be strings in the form {\"number\", \"string\", \"nil\"}")
			end
...
local function Check(PotentialTypes, Parameter, ArgumentNumOrName)
	...
	return false, "bad argument"
		...
		.. PotentialTypes[0] .. ", got " .. require(script.Parent.Debug).Inspect(Parameter)
end
	...
	__index = function(self, index)
		...
		for i in (index .. "Or"):gmatch("(%w-)Or") do -- Not the prettiest, but hey, we got parsing baby!
			...
			elseif i:find("^EnumerationOfType") then
				i = i:sub(18)
				local EnumerationType = require(script.Parent.Enumeration)[i]
...
function Typer.AssignSignature(...)
	...
	for a = 1, NumTypes do
		...
		if type(ParameterSignature) == "table" then
			...
			require(script.Parent.Debug).Error("Definition for parameter #" .. a .. " must be a table")
		end
	end
		...
		return function(...)
			...
			for a = 1, NumParameters < NumTypes and NumTypes or NumParameters do
				...
				elseif not Success then
					return require(script.Parent.Debug).Error(Error)
				end
			end
	...
	return function(Tab)
		...
		for Index in next, Tab do
			if not self[Index] then
				return false, "|Map.__call| " .. require(script.Parent.Debug).Inspect(Index) .. " is not a valid Key"
			end
		end
